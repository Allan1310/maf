...
import com.allinfnt.idc.common.security.Digests;
import com.allinfnt.idc.common.security.shiro.session.SessionDAO;
import com.allinfnt.idc.common.service.BaseService;
import com.allinfnt.idc.common.service.ServiceException;
...
import com.allinfnt.idc.modules.sys.dao.RoleDao;
...
import com.allinfnt.idc.modules.sys.security.SystemAuthorizingRealm;
import com.allinfnt.idc.modules.sys.utils.DictUtils;
import com.allinfnt.idc.modules.sys.utils.LogUtils;
...
public class SystemService extends BaseService implements InitializingBean {
	...
	public static boolean printKeyLoadMessage() {
		...
		sb.append("\r\n    欢迎使用 " + Global.getConfig("productName")
				+ "  - Powered By http://www.allinfnt.com\r\n");
		sb.append("\r\n======================================================================\r\n");
	 ...
	 * @return
	 */
	public List<User> findByPosition(String officeId, String positionCode) {
		if (officeId == null || StringUtils.isBlank(positionCode)) {
			return null;
		}
		String position = "%," + officeId + "_" + positionCode + ",%";
	...
	public User updateUserByNo(User user) {
		try {
			if(user!=null){
				Map<String,String> map = UserSynUtils.getIDCUserByUser(user.getNo());
				if(StringUtils.isNotEmpty(map.get("name"))){
					user.setName(map.get("name"));
				}
				if(StringUtils.isNotEmpty(map.get("loginName"))){
				...
				if(StringUtils.isNotEmpty(map.get("position"))){
					user.setPosition(map.get("position"));
				}
				if(StringUtils.isNotEmpty(map.get("departId"))){
					Office office = officeService.get(map.get("departId"));
				...
				user2.setLoginName(map.get("loginName"));
				user2.setEmail(map.get("email"));
				user2.setPhone(map.get("phone"));
				user2.setMobile(map.get("mobile"));
				user2.setUserType(map.get("userType"));
				user2.setPosition(map.get("position"));
				Office office = officeService.get(map.get("departId"));
	...
	public void updateUserAll() {
		...
		List<User> userList = userDao.findAllList(entity);
		for (User user : userList) {
			try {
				...
				if(map!= null && map.size()>0){
					...
					user.setPhone(map.get("phone"));
					user.setMobile(map.get("mobile"));
					String i =DictUtils.getDictValue(map.get("userType"), "sys_user_type", "");
					user.setUserType(DictUtils.getDictValue(map.get("userType"), "sys_user_type", ""));
					user.setPosition(map.get("position"));
					Office office = officeService.get(map.get("departId"));
					user.setOffice(office);
					userDao.update(user);
