...
import com.allinfnt.idc.common.utils.Collections3;
import com.allinfnt.idc.common.utils.StringUtils;
import com.allinfnt.idc.common.utils.TaskBean;
import com.allinfnt.idc.modules.act.dao.ActDao;
import com.allinfnt.idc.modules.act.dao.IdcTaskHastenDao;
...
import com.allinfnt.idc.modules.act.utils.ProcessDefCache;
...
public class ActTaskService extends BaseService {
	...
	@Autowired
	private IdcTaskHastenDao idcTaskHastenDao;
	...
	public static Map<String, ProcessDefinition> PROCESS_DEFINITION_CACHE = new HashMap<String, ProcessDefinition>();
	...
	public List<Act> todoList(Act act) {
		...
		for (Task task : todoList) {
			...
			if(StringUtils.isNotBlank(act.getTitle())){
				if(Canstants.getNotNullString(task.getProcessVariables().get("title")).indexOf(act.getTitle())>-1){
					Act e = new Act();
					e.setHisProcIns(processInstance);
					e.setTask(task);
	 ...
	 * @return
	 */
	public IdcTaskHasten getTaskHastenByTaskId(String taskId) {
		IdcTaskHasten taskHasten = idcTaskHastenDao.findByTaskId(taskId);
		if (taskHasten == null) {
			taskHasten = new IdcTaskHasten();
		}
		List<Task> tasks = taskService.createTaskQuery().processInstanceId(taskId).list();
		...
		taskHasten.setThisHastenUsers(Collections3.extractToString(users,"name", ", "));
		return taskHasten;
	}
	 ...
	 */
	@Transactional(readOnly = false)
	public void save(IdcTaskHasten hastenTask) {
		// 发催办邮件
		List<Task> tasks = taskService.createTaskQuery()
				.processInstanceId(hastenTask.getTaskId()).list();
		List<User> users = new ArrayList<User>();
		for (Task task : tasks) {
			users.add(UserUtils.getByLoginName(task.getAssignee()));
			String processDefinitionId = task.getProcessDefinitionId();
			ProcessDefinition def = this.getProcessDefinition(processDefinitionId);
			HistoricProcessInstance hisProcessInstance = historyService.createHistoricProcessInstanceQuery()
					.processInstanceId(hastenTask.getTaskId()).singleResult();
			
			TaskBean tb = new TaskBean();
			tb.setTask(task);
			tb.setHistoricProcessInstance(hisProcessInstance);
			tb.setProcessDefinition(def);
			tb.setHastenTask(hastenTask);
			
			sendNotifyMessage(UserUtils.getByLoginName(task.getAssignee()), tb);
		}
		hastenTask.setLastHastenUsers(Collections3.extractToString(users,"name", ", "));
		if (StringUtils.isBlank(hastenTask.getId())){
			hastenTask.preInsert();
			idcTaskHastenDao.insert(hastenTask);
		}else{
			hastenTask.preUpdate();
			idcTaskHastenDao.update(hastenTask);
		}
	}
	 ...
	 * @param tb
	 */
	private void sendNotifyMessage(User user, TaskBean tb) {
		try {
			Map<String, String> map = Maps.newHashMap();
			String title = "工作催办：【" + tb.getProcessDefinition().getName() + "-"
					+ tb.getTask().getName() + "】需要您来处理";
			map.put("subject", title);
			map.put("template", "mailBmpTemplate.ftl");
			map.put("mailTo", user.getEmail());
			map.put("userName", user.getName());
			map.put("message", getMessage(tb));
			map.put("sendEmail", "true");
			
			sendMessage(user,map,title);
		} catch (Exception e) {
	...
	private String getMessage(TaskBean tb) {
		
		// 获取流程XML上的表单KEY
		String formKey = this.getFormKey(tb.getTask().getProcessDefinitionId(),tb.getTask().getTaskDefinitionKey());
		...
		message.append("?id=");
		message.append(tb.getHistoricProcessInstance().getBusinessKey());
		message.append("&taskid=");
		message.append(tb.getTask().getId());
		message.append("&tkey=");
		message.append(tb.getTask().getTaskDefinitionKey());

		message.append("&category=inbox\'>");
		message.append(tb.getTask().getName());
		message.append("</a></br></br>");

		if (tb.getHastenTask().getCount() > 1) {
			message.append(tb.getHastenTask() + "</br></br>");
		}
		
		Map<String, Object> datamap = (Map<String, Object>) tb.getTask().getProcessVariables().get("datamap");
		
		if (datamap == null) {
			List<HistoricVariableInstance> listVariables = historyService
					.createHistoricVariableInstanceQuery()
					.processInstanceId(tb.getProcessInstanceId()).list();
	 ...
	 * @return
	 */
	public Page<Act> listRunningProcessInstances(Page<Act> page ){
		
		List<HistoricProcessInstance> historicTasks = historyService
				.createHistoricProcessInstanceQuery()
		...
		HistoricProcessInstanceQuery processInstanceQuery = historyService
				.createHistoricProcessInstanceQuery().startedBy(UserUtils.getUser().getLoginName())
				.unfinished();

		List<Act> acts = Lists.newArrayList();
		for (HistoricProcessInstance task : historicTasks) {
			
			String processDefinitionId = task.getProcessDefinitionId();
			
			ProcessDefinition def = this.getProcessDefinition(processDefinitionId);
			Task tk = taskService.createTaskQuery().processInstanceId(task.getId()).singleResult();
			Act tb = new Act();
			tb.setTask(tk);
			tb.setVars(taskService.getVariables(tk.getId()));
			tb.setHisProcIns(task);
			tb.setProcInsId(task.getId());
			tb.setProcDef(def);
			IdcTaskHasten hastenTask = this.getTaskHastenByTaskId(task.getId());
			tb.setHastenTask(hastenTask);
			acts.add(tb);
		}
	 ...
	 * @return
	 */
	public Page<Act> listCompletedProcessInstances(Page<Act> page ){
		...
		List<Act> taskBeans = Lists.newArrayList();
		for (HistoricProcessInstance task : historicTasks) {
			Map<String, Object> vars = Maps.newHashMap();
			List<HistoricVariableInstance> tks  = historyService.createHistoricVariableInstanceQuery().processInstanceId(task.getId()).list();
			if(tks.size()>0){
				for(HistoricVariableInstance tk:tks){
					vars.put(tk.getVariableName(), tk.getValue());
				}
			}
	 ...
	 * @return
	 */
	public String getWithDrawMessage(Task task,User user){
		StringBuffer message = new StringBuffer("");
		// 获取流程XML上的表单KEY
		String formKey = this.getFormKey(task.getProcessDefinitionId(),task.getTaskDefinitionKey());
		message.append(user.getName()+"已撤销【" + task.getName() + "】任务,进行重新申请，请知晓！");
		message.append("<a href='" + StringUtils.getHost() + "/");
		message.append(formKey);
		message.append("?id=");
		String[] busKeyStrs = task.getId()
				.split("_");
	 ...
	 */
	@Transactional(readOnly = false)
	public void sendMessage(User user ,Map<String, String> map ,String title){
		String mailMsg;
		try {
			mailMsg = mailService.generateContent(map);
	 ...
	 * @return
	 */
	public List<CmBaseCode> findBaseCodesByName(CmBaseCode baseCode){
		return cmBaseCodeDao.findList(baseCode);
