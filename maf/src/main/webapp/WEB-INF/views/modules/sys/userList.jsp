	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnSyn").click(function(){
				top.$.jBox.confirm("确认要同步所有用户数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/sys/user/synAll");
						$("#searchForm").submit();
					}
				},{buttonsFocus:1});
				top.$('.jbox-body .jbox-icon').css('top','55px');
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/list" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="return page();"/>
				...
				<input id="btnSyn" class="btn btn-primary" type="button" value="同步"/></li>
			<li class="clearfix"></li>
		</ul>
	</form:form>
