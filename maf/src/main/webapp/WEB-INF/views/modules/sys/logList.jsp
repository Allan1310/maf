...
<!-- 	</ul> -->
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			...
			<label>操作用户：</label> <sys:treeselect id="user" name="createBy.id" value="${log.createBy.id}" labelName="createBy.name" labelValue="${log.createBy.name}"
					title="操作用户" url="/sys/office/treeData?type=3" cssClass="input-small"   allowClear="true" notAllowSelectParent="true"/>
