...
<head>
	...
	<script type="text/javascript">
		...
		function checked(){
			/* var tempVals = $("#officeId").val();
			
			if(tempVals == null || tempVals == ''){
				$.jBox.error("请选择上级机构","提示");
				$("#officeName").focus();
				return false;
			} */
			var tempVals = $("#primaryPersonId").val();
			if(tempVals == null || tempVals == ''){
				$.jBox.error("请选择主要负责人","提示");
			...
			tempVals = $("#code").val();
			if(tempVals != null && tempVals != ''){
				if(!checkCode(tempVals)){
					return false;
				}
			}
			return true;
		}
		function checkCode(objs){
			var flag = true;
			var isCheck = true;
			var officeId = $("#id").val();
			if(officeId != null && officeId != ''){
				var code =  $.trim($("#code").val());
				var tempCode =  $.trim($("#tempCode").val());
				if(code == tempCode){
					isCheck = false;
				}
			}else{
			...
			if(isCheck){
				$.ajax({
					type : 'POST',
					async:false,
					url : '${ctx}/sys/office/checkOnlyCode',  
					data : {code:objs},
					dataType:'json',
...
<body>
	...
	<form:form id="inputForm" modelAttribute="office" action="${ctx}/sys/office/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input type="hidden" id = "tempCode" value="${office.code }"/>
