...
<script type="text/javascript">
		$(document).ready(function() {
			$(".label").each(function(i,o){
				...
				<ul id="menu-${param.parentId}" class="accordion">
					<c:set var="menuList" value="${fns:getMenuList()}"/>
					<c:set var="firstMenu" value="true"/>
					<c:forEach items="${menuList}" var="menu" varStatus="idxStatus">
						<c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&&menu.isShow eq '1'}">
							<li class="submenu ${not empty firstMenu && firstMenu ? 'open' : ''}">
								<a  data-parent="#menu-${param.parentId}" data-href="#collapse-${menu.id}" href="#collapse-${menu.id}" title="${menu.remarks}"><i class="icon icon-${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'}"></i> <span>&nbsp;${menu.name}</span> <span class="label">2</span></a>
								<ul id="collapse-${menu.id}" class="submenu-body">
									<c:forEach items="${menuList}" var="menu2">
										<c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
											<li><a  data-href=".menu3-${menu2.id}" href="${fn:indexOf(menu2.href, '://') eq -1 ? ctx : ''}${not empty menu2.href ? menu2.href : '/404'}" target="${not empty menu2.target ?menu2.target: (menu2.href eq '/logout'?'':'mainFrame')}"><i class="icon icon-${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'}"></i><span>&nbsp;${menu2.name}</span></a></li>
