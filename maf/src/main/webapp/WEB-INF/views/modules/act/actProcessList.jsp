		...
		<tbody>
			<c:forEach items="${page.list}" var="object">
				...
				<tr>
					...
					<td title="${process.key}">${process.name}</td>
					<td><b title='${process.id}'>V${process.version}</b></td>
					<!-- <td><a target="_blank" href="${ctx}/act/process/resource/read?procDefId=${process.id}&resType=xml">${process.resourceName}</a></td> -->
					<td><a target="_blank" href="${ctx}/act/process/resource/read?procDefId=${process.id}&resType=image">查看</a></td>
